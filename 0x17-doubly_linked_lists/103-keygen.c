...
int f4(char *usrn, int len)
{
    ...
    {
        if (max_char < usrn[current_char])
            max_char = usrn[current_char];
        current_char += 1;
    }
...
int f5(char *usrn, int len)
{
    ...
    while (current_char < len)
    {
        result = result + usrn[current_char] * usrn[current_char];
        current_char += 1;
    }
...
int f6(char *usrn)
{
    ...
    rand_char = current_char = 0;

    while (current_char < *usrn)
    {
        rand_char = rand();
        current_char += 1;
    }
